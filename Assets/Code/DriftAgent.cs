...
public class DriftAgent : Agent
{
  ...
  private float[] curvatureRange = new float[2]{-18.2f, 18.2f};
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if(fixedPathIndex != -1 && fixedStartDistancesOnPaths[fixedPathIndex] != 0){
            startDistanceOnPath = fixedStartDistancesOnPaths[fixedPathIndex];
        }

        //initPos 
        direction = Random.Range(0, 2) * 2 - 1;
        if(fixedPathIndex != -1 && fixedDirections[fixedPathIndex] != 0){
    ...
    private float DriftReward(float distanceOnPath) {
        ...
        float driftReward = Mathf.Abs(slipAngle) / maxSlip;  // Normalize slip angle
        
        // Ensure driftReward is within [0, 1]
        driftReward = Mathf.Clamp(driftReward, 0f, 1f);
